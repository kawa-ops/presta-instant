...
export async function GET() {
  ...
  try {
    const users = await db.user.findMany({
      ...
      select: { id: true, name: true, email: true, phone: true, siret: true, specialty: true, address: true, profilePicUrl: true, active: true, createdAt: true, rates: true, unavailableDates: true },
      orderBy: { name: 'asc' },
    })
